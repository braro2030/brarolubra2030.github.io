...
function mostrarControles(serviciosSeleccionados) {
    ...
    serviciosSeleccionados.forEach(servicio => {
        const tareas = tareasPorServiciosaas[servicio];
    ...
    Object.keys(resultadosPorServicio).forEach(servicio => {
        ...
        resultadosPorServicio[servicio].forEach(resultado => {
            if (resultado.cumplimiento === "No Cumple" || resultado.cumplimiento === "Cumple Parcialmente") {
                const recomendaciones = recomendacionesPorServiciosaas[resultado.servicio].filter(rec => rec.id === resultado.id);
...
function dibujarGraficaPolarTotal(resultados) {
    ...
    const tituloGrafica = document.createElement('h3');
    tituloGrafica.textContent = 'Cumplimiento de Controles por Servicio';
    tituloGrafica.style.textAlign = 'center';
    graficasContainer.appendChild(tituloGrafica);
